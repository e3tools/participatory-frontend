...
const ResourcesDefault = () => {
  ...
  let db = new DocTypeService('Help Resource');
  ...
  useEffect(() => {
    ...
    matching = data?.filter(
      (el) =>
        el?.title
          ?.toLocaleLowerCase()
          .indexOf(search_query?.toLocaleLowerCase()) > -1,
    );
            ...
            <Card.Content>
              ...
              >
                ...
                <AppButton
                  style={{ flex: 1 }}
                  mode="outlined"
                  label={APP._('BUTTON.SEE_DETAILS')}
                  onPress={() => {
                    APP.navigateToPath(
                      navigation,
                      '/screens/resources/detail',
                      {
                        docname: el.name,
                      },
                    );
