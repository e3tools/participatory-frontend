...
const InterventionType = (props) => {
  return (
    <>
      <PageIntro text={APP._('MAP_PAGE.SWIPE')} showImage={false} />
      <Carousel
        loop={false}
        width={props.width}
        height={props.height}
...
const AcaciaWater = (props: AcaciaWaterType) => {
  ...
  return is_loading ? (
    ...
    <ScrollView contentContainerStyle={styles.interventions_container}>
      {interventions.map((el, idx) => (
        <InterventionItem key={`${el.category}${idx}`} item={el} />
      ))}
    </ScrollView>
  );
};
