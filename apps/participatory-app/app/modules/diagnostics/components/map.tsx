...
import ImageOverlay from 'gis/components/layers/image-overlay';
import WMSTileLayer from 'gis/components/layers/wms-tile-layer';
import MapQueryDialog from './map-query-dialog';
...
import AppSelect from 'ui/components/form/controls/select';
...
import { TechnicalAnalysisMenuItem } from './technical-analysis-menu-item';
import { DATASOURCE } from 'gis/enums';
import Carousel from 'react-native-reanimated-carousel';
import { AppIconButton } from 'ui/components/shared/app-icon-button';
import AcaciaWater from './acacia-water';
import SLMHelp from './slm-help';
...
const AppMap = class AppMap extends React.Component<IMapProps> {
  ...
  focus_region = (region: Region, zoom?: number) => {
    ...
    this.map_ref.current?.animateCamera(
      { center: region, zoom: zoom || 5 },
      { duration: 3000 },
    );
  };
   ...
   * @param markers
   */
  add_markers = (markers: IMarkerProps[]) => {
  ...
  add_image_overlay = (name: string, overlay: IImageOverlayProps) => {
    const index = this.state.image_overlays.findIndex(
      (itm) => itm.name === name,
    );
  ...
  select_feature = async (id: string, level: number, layer: object) => {
    ...
    if (!feature) {
      ...
      if ('features' in admin_geom) {
        let added_layer = this.add_feature(
          admin_geom['features'][0],
          SELECTED_VECTOR_LAYER_NAME /*admin.name*/,
        );
      } else {
        let added_layer = this.add_feature(
          { geometry: admin_geom },
  ...
  toggle_analysis = async (analysis: object, checked: boolean) => {
    ...
    if (
      /*analysis?.datasource_type == DATASOURCE.RASTER  &&*/ !this.state
        .selected_admin
    ) {
    ...
    if (exists) {
      ...
      const ref =
        datasource_type === DATASOURCE.RASTER
          ? createRef<typeof WMSTileLayer>()
          : datasource_type === DATASOURCE.VECTOR
            ? createRef<typeof GeoJsonLayer>()
            : createRef();
      analysis['ref'] = ref;
  ...
  add_analysis = (
    ...
    const analysis = TechnicalAnalysisService.get_analysis(analysis_name).then(
      (doc) => {
        //   remove_analysis(doc.analysis_name);

        TechnicalAnalysisService.get_computation(
          doc.name,
          vector_id,
          admin_level,
        ).then((res) => {
  ...
  take_snapshot_and_share = async () => {
    const snapshot = await this.map_ref?.current?.takeSnapshot({
      width: 300,
      height: 300,
      result: 'base64',
    });
  ...
  _set_layer_ref = (
    ref_id: string,
    ref_obj: React.RefObject<React.FunctionComponent>,
  ) => {
  ...
  update_analysis_results = (analysis_name: string, results: {}) => {
    let index = this.state.selected_analyses?.findIndex(
      (el) => el.name === analysis_name,
    );
    if (index != -1) {
      let next_state = this.state.selected_analyses;
      ...
      <View>
            ...
            <View style={styles.toolbar}>
              ...
              <View style={[styles.toolbar_container, { flex: 1 }]}>
                <View style={styles.options_container}>
                  <Switch
                    value={this.state.slm_enabled}
                    onValueChange={() =>
                      this.setState({
                        slm_enabled: !this.state.slm_enabled,
                        slm_snack_bar_visible: true,
                      })
                    }
                  />
        ...
        <MapView
          ...
          onPress={(e) => {
            ...
            this.setState({
              selected_point: { lat: coords.latitude, lon: coords.longitude },
            });
          }}
          // onPoiClick={(e) => {
        ...
        >
          ...
          {this.state.slm_enabled === true && this.state.selected_point && (
            <MapMarker
              title=""
              location={{
        ...
        {this.state.slm_enabled === true && (
          <Text style={styles.coordinates}>
            {this.state.selected_point?.lat} , {this.state.selected_point?.lon}
          </Text>
        )}
        {this.state.slm_enabled === true && (
          <AcaciaWater point={this.state.selected_point} />
        )}
                  ...
                  >
                    <Card>
                      <Card.Title
                        ...
                        left={(props) => (
                          <Avatar.Icon {...props} icon="map-legend" />
                        )}
                        right={(props) => (
                          <IconButton
                            {...props}
                            ...
                            onPress={() => {
                              this.setState({
                                active_analysis_visible: true,
                                active_analysis: item,
                              });
                            }}
                          />
                        )}
        ...
        )}
        {
          <Portal>
            <Snackbar
              ...
              // }}
            >
              {this.state.slm_enabled
                ? APP._('MAP_PAGE.MESSAGES.SLM_ENABLED')
                : APP._('MAP_PAGE.MESSAGES.SLM_DISABLED')}
            </Snackbar>
          </Portal>
        }
        {this.state.active_analysis && this.state.active_analysis_visible && (
          <Portal>
            <Dialog
              visible={this.state.active_analysis_visible}
              onDismiss={() =>
                this.setState({ active_analysis_visible: false })
              }
            >
                    ...
                    <Card>
                        <Card.Content>
                            <List.Item
                                ...
                                right={props => <View>
                                    <AppButton label={APP._('BUTTON.OK')} onPress={() => { 
                                    }}/> 
                                </View>} 
                            >
                            </List.Item>
