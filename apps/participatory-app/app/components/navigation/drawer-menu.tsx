...
import { getDashboards } from '@/app/state/dashboard/actions/dashboard.action';
import { getEngagements } from '@/app/state/engagement/actions/engagement.action';
...
export default function DrawerMenu(props: any) {
  ...
  const isAuthenticated = useAuthSelector(
    (state) => state.user?.loggedInUser !== null,
  );
  ...
  const initial_setup = () => {
    ...
    //   });
    // }
    if (isAuthenticated /*auth.is_authenticated*/) {
      // load_engagements();
      // load_dashboards();
  ...
  useEffect(() => {
    initial_setup(); //do this to trigger dashboards and other restricted items
  ...
  const navigate_engagement = (props, engagement: object) => {
    ...
    if (engagement.has_data_forms) {
      APP.navigateToPath(props.navigation, 'views/list/[doctype]', {
    ...
    <View style={{ flex: 1 }}>
      <DrawerContentScrollView {...props}>
        <View style={{ padding: 20 }}>
          <TouchableOpacity
            onPress={() =>
              APP.navigateToPath(props.navigation, 'screens/home_screen')
            }
          >
            <Image
        ...
        >
          {engagements?.map((engagement) => (
            <List.Item
              ...
              style={styles.drawer_child}
              onPress={() => {
                navigate_engagement(props, engagement);
              }}
              onLongPress={() => {}}
            />
          ))}
        ...
        {/* <List.Item 
            ...
            onPress={()=>{  
              // APP.navigateToPath(props.navigation, 'views/list/[doctype]', { doctype: 'Engagement Action Task' })
        ...
        <List.Item
          ...
          onPress={() => {
            APP.navigateToPath(
              props.navigation,
              'modules/diagnostics/screens/DiagnosticsScreen',
              {},
            );
          }}
          onLongPress={() => {}}
        ...
        {/* Action Plans
          <List.Accordion
            ...
            <List.Item 
              ...
              style={ styles.drawer_child }
              onPress={()=>{ 
                APP.navigateToPath(props.navigation, 'views/form/index', { 
        ...
        <List.Item
          ...
          onPress={() => {
            APP.navigateToPath(
              props.navigation,
              'modules/resources/screens/index',
              {},
            );
        ...
        >
          <List.Item
            ...
            style={styles.drawer_child}
            onPress={() => {
              APP.navigateToPath(props.navigation, 'views/report/[doctype]', {
                doctype: 'User',
              });
        ...
        <List.Item
          ...
          onPress={() => {
            APP.navigateToPath(
              props.navigation,
              'modules/auth/screens/user_profile_screen',
              {},
            );
          }}
          onLongPress={() => {}}
