import { set_value, get_value, delete_value } from 'common/utils/storage';
...
const EngagementStore = class EngagementStore {
  ...
  static set_current_engagement = async (engagement: object) => {
    let obj = {
      ...
      has_data_forms: engagement.has_data_forms,
    };
   ...
   * @param data
   */
  static set_survey_form_data = (
    engagement: string,
    doctype: string,
    data: object,
  ) => {
    console.log('Setting survey data: ', engagement, doctype, data);
    this._setFormValue(engagement, SURVEY, doctype, data);
  };
   ...
   * @returns
   */
  static getSurveyFormData = (engagement: string, doctype: string) => {
    return this._getFormValue(engagement, SURVEY, doctype);
  };
  /**
   * Delete engagement from local storage
   * @returns
   */
  static removeEngagement = async () => {
   ...
   * @returns
   */
  static getSurveyEngagementEntryData = (engagement: string) => {
   ...
   * @returns
   */
  static set_survey_engagement_entry_data = (
    engagement: string,
    data: object,
  ) => {
   ...
   * @returns
   */
  static _get_value_by_engagement_type = (
    engagement: string,
    engagement_type: string,
  ) => {
   ...
   * @returns
   */
  static _set_Value_by_engagement_type = (
    engagement: string,
    engagement_type: string,
    data: object,
  ) => {
